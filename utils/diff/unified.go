...
func Unified(oldLabel, newLabel, oldText, newText string) Diffs {
	edits := Strings(oldText, newText)

	unified, err := ToUnified(oldLabel, newLabel, oldText, edits)
	if err != nil {
...
func toUnified(fromName, toName string, content string, edits []Edit) (Diffs, error) {
	u := Diffs{
		From: fromName,
		To:   toName,
	}
